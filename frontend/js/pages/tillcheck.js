...
export async function tillcheckPage({ user }) {
  ...
  async function load(next = page) {
    ...
    mount('TillCheck', 'Understand a purchase before you spend. Your emergency money stays protected.', summaryCards(summary) + `<section class="content-grid-2">` +
      ...
      card('Check history', (history.rows.length ? history.rows.map(row => `<div class="record-row"><div><strong>${e(row.item_name)} · ${e(money(row.amount))}</strong><small>${e(dateLabel(row.created_at))} · ${row.was_purchased ? 'Purchased' : 'Not recorded as purchased'}</small></div>${button('View check', `view-${row.check_id}`)}</div>`).join('') : empty('Your checks will appear here.')) + pagination(page, history.hasMore)) + '</section>');
