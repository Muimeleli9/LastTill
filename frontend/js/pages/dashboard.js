...
export async function dashboard({ user, profile }) {
  ...
  async function load(next = page) {
    ...
    mount(`Hello, ${profile.full_name.split(' ')[0]}`, 'A clear view of what you can spend, save, and protect.',
      ...
      card('Your spending plan', `<p class="goal-note">${Number(summary.remaining) < 0 ? 'Your cycle has a shortfall. Review expenses and income before taking on more spending.' : 'Your daily allowance excludes money reserved for savings and emergency deposits.'}</p>${stat('Reserved this cycle', money(Number(summary.savings) + Number(summary.deposits)))}`,'card-green') +
